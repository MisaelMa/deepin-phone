...
*/
#ifndef PHONEICONFILEMODEL_H
#define PHONEICONFILEMODEL_H

#include "base/BaseItemModel.h"

/**
 * @brief 手机助手 icon 对象模型
 */

class PhoneIconFileModel : public BaseItemModel
{
    Q_OBJECT
public:
    explicit PhoneIconFileModel(QObject *parnet = nullptr);

    // BaseItemModel interface
public:
    void appendData(const QVariant &) override;
    void appendData_FI(const FILE_INFO &file_info); //向model添加数据
