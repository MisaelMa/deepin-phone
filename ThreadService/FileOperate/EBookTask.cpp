...
EBookTask::EBookTask(QObject *parent)
...
void EBookTask::run()
{
    ...
    getEBookInfoLevel(m_strLoadDirPath, 1);
    if (m_bsTaskCanRun) {
...
//检索所有目录下的电子书文件，为了提高速度，目前只检索从根目录开始的4层
void EBookTask::getEBookInfoLevel(QString path, int dirLevel)
{
    ...
    for (auto file : fileList) {
        ...
        if (file.isDir()) {
            if (ebookSkipDir.contains(file.fileName()))
                continue;

            getEBookInfoLevel(file.absoluteFilePath(), dirLevel + 1);
        } else {
            QString suffix = file.suffix();
            if (ebookSuffix.contains(suffix, Qt::CaseInsensitive)) {
